...
void init_vars()
{
  ...
  for (uint16_t adc = 1000; adc < 4096; adc++)
  {
    ...
    if (_get(VAR_ADC_VDDL) == 0 && v >= SUPPLY_VLOW / 2)
      _set(VAR_ADC_VDDL, adc);
    if (_get(VAR_ADC_VDDH) == 4095 && v >= SUPPLY_VHIGH / 2)
      _set(VAR_ADC_VDDH, adc);
  }
}
...
void init_adc()
{
  ...
  adc1_config_channel_atten(VDD_CHANNEL, ADC_ATTEN_DB_12);
  int sum = 0;
  for (int i = 0; i < 8; i++)
    sum += adc1_get_raw(VDD_CHANNEL);
...
void load_config(int whichvars = 0)
{
  ...
  if (whichvars != SKIP_RTC_VARS)
  {
    ...
    if (dict["tickpin"].is<int>())
    {
      _set(VAR_TICKPIN, dict["tickpin"]);
    }
    if (dict["tune_level"].is<int>())
    {
      _set(VAR_TUNE_LEVEL, dict["tune_level"]);
      _set(VAR_SLEEP_INTERVAL, TUNE_INTERVALS[_get(VAR_TUNE_LEVEL)]);
    }
    if (dict["ulp_timer"].is<int>())
    {
...
void tune_ulp_timer()
{
  ...
  if (abs(diff) <= 5)
    return; // Do not adjust ULP timer if net time is off by <= 5secs
...
void startup()
{
  ...
  init_adc();
  _set(VAR_ADC_VDD, _get(VAR_ADC_VDDH));
  init_gpio();
