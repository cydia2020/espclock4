// Source: https://www.esp32.com/viewtopic.php?t=7023
//
// - Define ULP_RESERVE_MEM = 8000 so ULP code can go up to 2000 instructions
// - Rename ulp_process_macros_and_load() => patched_ulp_process_macros_and_load().
#define ULP_RESERVE_MEM (1800 * 4)
 ...
 */
#define RELOC_INFO_LABEL(label_num, insn_addr) \
    (reloc_info_t)                             \
    {                                          \
        .label = label_num,                    \
        .addr = insn_addr,                     \
 ...
 * of label label_num.
 */
#define RELOC_INFO_BRANCH(label_num, insn_addr) \
    (reloc_info_t)                              \
    {                                           \
        .label = label_num,                     \
        .addr = insn_addr,                      \
 ...
 * at label label_num.
 */
#define RELOC_INFO_LABELPC(label_num, insn_addr) \
    (reloc_info_t)                               \
    {                                            \
        .label = label_num,                      \
        .addr = insn_addr,                       \
                                 ...
                                 reloc_info_t label_info, reloc_info_t the_reloc)
{
    ...
    case RELOC_TYPE_BRANCH:
    {
        ...
        assert(insn->b.opcode == OPCODE_BRANCH && "branch macro was applied to a non-branch instruction");
    ...
    {
        assert((insn->alu_imm.opcode == OPCODE_ALU && insn->alu_imm.sub_opcode == SUB_OPCODE_ALU_IMM && insn->alu_imm.sel == ALU_SEL_MOV) && "pc macro was applied to an incompatible instruction");
